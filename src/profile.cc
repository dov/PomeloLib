...
// The tangent at the end of the last bezier segment. That is the
// direction of the last control point to the last node, and only when the
// control point sits on top of the node does the segment's other end have
// to stand in for it.
Vec2 LayerData::get_end_dir() const
{
  if (this->size() < 2)
    return Vec2(0,0);

  const NodeData& last = this->back();
  Vec2 dir = last.xy - last.control_before_xy;
  if (fabs(dir.x) < 1e-12 && fabs(dir.y) < 1e-12)
    dir = last.xy - (*this)[this->size()-2].xy;

  return dir;
}


void ProfileData::scale(double sx, double sy)
{
  for (auto& layer : *this)
    for (auto& node : layer)
    {
      node.xy.x *= sx;                 node.xy.y *= sy;
      node.control_before_xy.x *= sx;  node.control_before_xy.y *= sy;
      node.control_after_xy.x *= sx;   node.control_after_xy.y *= sy;
    }
}
...
void ProfileData::load_from_string(const string& profile_string)
{
  ...
  if (!j.contains("layers") || !j["layers"].is_array())
    throw runtime_error("a profile needs a \"layers\" array");

  const json& layers = j["layers"];
  this->resize(layers.size());

  for (size_t i = 0; i<layers.size(); i++)
  {
    try {
      (*this)[i].from_json(layers[i]);
    }
    catch (const std::exception& exc) {
      throw runtime_error(fmt::format("profile layer {}: {}", i, exc.what()));
    }
  }
}
