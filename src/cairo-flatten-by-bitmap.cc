                                        ...
                                        double resolution)
{
  ...
  // trace_image() emits the traced path in bitmap coordinates scaled by
  // 1/resolution, i.e. relative to the ink bounding box corner that was
  // subtracted when the recording surface was painted above. Translate it
  // back so that the flattened path lands in the same coordinate frame as
  // the paths that went in. Cairo stores the path in device space, so
  // building it under the shifted matrix and then restoring gives the
  // caller its coordinates back in the original user space.
  cairo_save(m_ctx);
  cairo_translate(m_ctx, x0, y0);
