...
class TeXtrusion {
  ...
  // constructor
    ...
    // Tolerance the bezier profile is flattened to. It is an absolute
    // length, so a caller that scales the profile has to scale this too.
    double profile_linear_limit = 0.01;

    // Nearby vertices are welded at the end of skeleton_to_mesh() to work
    // around calculation imprecision. Also an absolute length: too tight
    // and coincident vertices stay apart, leaving cracks in the surface.
    double merge_distance = 1e-4;
    double profile_radius = 3.0;
    double profile_round_max_angle = G_PI/2;
  ...
  private:
    ...
    void add_region_contribution_cap_to_mesh(
      ...
      // The slope to extrapolate along past the end of flat_list. NAN
      // when the caller has none to offer, and then the slope of the
      // flattened curve stands in for it.
      double end_slope,
