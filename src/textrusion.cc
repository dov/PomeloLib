...
// One edge of a region's offset curve at a fixed distance from the
// boundary, together with the third vertex of the triangle it came from,
// which lies further in and so tells us which way is outwards.
namespace {
struct RingSegment {
  Point3 a, b, inner;
};
...
// The slope the roof continues at past the end of a profile layer, from
// the layer's own end tangent. NAN when the layer has no tangent to give
// or leaves vertically, and then the cap falls back to the slope of the
// flattened curve.
static double layer_end_slope(const LayerData& layer)
{
...
// Recover the ring of segments at a given inset. A region can only hand
// out offset curves over an interval, so take a thin slice just inside the
// requested distance and keep the edges that lie exactly on it.
static vector<RingSegment> region_ring_at(
  const SkeletonPolygonRegion& region,
  double offs)
{
  vector<RingSegment> ring;

  const double depth = region.get_depth();
  const double delta = (depth-offs)*0.01;
  if (delta <= 0)
    return ring;

  auto pp = region.get_offset_curve_and_triangulate(offs, offs+delta);
      ...
      for (int i=0; i<3; i++)
        {
          ...
          const auto& c = poly[(i+2)%3];
          if (fabs(a.z()-offs) < eps && fabs(b.z()-offs) < eps)
            ring.push_back({a,b,c});
        }
    }
  return ring;
}

// Close a vertical step in the profile.
//
// Flattening a profile that turns around in z can put two consecutive
// samples at the same inset with different heights. That is a wall, but it
// makes for a zero width band, and the offset curve between a distance and
// itself is empty - so the two heights used to be left unconnected and the
// surface stood open all the way around the region. A wavy, non monotone
// profile hits this at its turning points.
static void add_profile_step_wall_to_mesh(
  Mesh& mesh,
  const vector<RingSegment>& ring,
  double z_start,
  double z_end,
  bool is_lower)
{
  for (const auto& s : ring)
    {
      // Mesh space, remembering that y is negated on the way in.
      glm::dvec3 a0 { s.a.x(), -s.a.y(), z_start };
      glm::dvec3 b0 { s.b.x(), -s.b.y(), z_start };
      glm::dvec3 a1 { s.a.x(), -s.a.y(), z_end };
      glm::dvec3 b1 { s.b.x(), -s.b.y(), z_end };

      // The wall faces outwards where the profile steps up going inwards
      // and inwards where it steps down, and the whole thing flips again
      // on the underside.
      glm::dvec3 mid = 0.5*(a0+b0);
      glm::dvec3 inward { s.inner.x() - mid.x, -s.inner.y() - mid.y, 0.0 };
      glm::dvec3 want = (z_end > z_start) ? -inward : inward;
      if (is_lower)
        want = -want;

      glm::dvec3 n = glm::cross(b0-a0, b1-a0);
      if (glm::dot(n, want) >= 0)
        {
          mesh.vertices.push_back(a0); mesh.vertices.push_back(b0);
          mesh.vertices.push_back(b1);
          mesh.vertices.push_back(a0); mesh.vertices.push_back(b1);
          mesh.vertices.push_back(a1);
        }
      else
        {
          mesh.vertices.push_back(a0); mesh.vertices.push_back(b1);
          mesh.vertices.push_back(b0);
          mesh.vertices.push_back(a0); mesh.vertices.push_back(a1);
          mesh.vertices.push_back(b1);
        }
    }
}
...
void TeXtrusion::add_region_contribution_cap_to_mesh(
  ...
  double end_slope, // The slope to extrapolate along, or NAN for the fallback
  ...
  for (size_t d_idx=0; d_idx<flat_list.size(); d_idx++)
    {
      ...
      if (d_idx == flat_list.size()-1)
        {
          ...
          // Continue along the direction the profile leaves its last node
          // in, which the caller takes from the authored curve rather than
          // from its flattening. A chord of a curve that is levelling out
          // still slopes upwards, so estimating the direction from the
          // flat list sent the roof of a profile ending horizontally on
          // climbing right through whatever sits above it.
          double slope = end_slope;
          if (std::isnan(slope))
            {
              // A single point carries no direction to extrapolate along.
              if (d_idx < 1)
                continue;

              // Two points back where there are that many, which is what a
              // flattened curve gives. A straight profile segment, though,
              // flattens to just its two endpoints - cairo has nothing to
              // subdivide - and then the last segment is the only slope
              // there is. Indexing d_idx-2 regardless used to underflow and
              // abort the bake on any linear profile.
              auto dxy = (d_idx >= 2
                          ? flat_list[d_idx-1] - flat_list[d_idx-2]
                          : flat_list[d_idx]   - flat_list[d_idx-1]);
              slope = dxy.y/dxy.x;
            }
      ...
      // A vertical step in the profile. See add_profile_step_wall_to_mesh().
      if (offs_end - offs_start < epsilon)
        {
          if (fabs(z_end - z_start) > epsilon)
            {
              auto ring = region_ring_at(region, offs_start);
              add_profile_step_wall_to_mesh(mesh, ring, z_start, z_end,
                                            is_lower);

              // The outer tube hangs off the first entry, so it still
              // needs its ring even when that entry is a wall.
              if (d_idx == 0)
                for (const auto& s : ring)
                  connection_rings.push_back(
                    { Vec3 { s.a.x(), -s.a.y(), flat_list[0].y },
                      Vec3 { s.b.x(), -s.b.y(), flat_list[0].y } });
            }
          continue;
        }
  ...
  )
{
  ...
  if (!region.polygon.is_simple()) {
    // Dropping a region leaves a hole through the solid, so this needs to
    // be visible rather than lost on stderr.
    spdlog::error("Non simple region ph {} region {} (layer {}) skipped in "
                  "the cap - this leaves a hole in the mesh",
                  ph_idx, r_idx, layer_idx);
    return;
  }
  ...
  )
{
        ...
        for (auto &r : phi.regions)
          {
            ...
            if (!r.polygon.is_simple())
              {
                spdlog::error("Non simple region ph {} region {} skipped - "
                              "this leaves a hole in the mesh",
                              ph_idx, r_idx);
                continue;
              }
            ...
            if (this->use_profile_data)
                ...
                for (size_t layer_idx=0; layer_idx< this->profile_data.size(); layer_idx++)
                  {
                    ...
                    layer.set_linear_limit(this->profile_linear_limit);
                    auto flat_list = layer.get_flat_list();
                    double end_slope = layer_end_slope(layer);
                    double prev_end_slope;
                    ...
                    if (layer_idx > 0)
                    {
                      ...
                      // Which only happens when the base layer ran out
                      // before this one did, so its last point really is
                      // its end and its own tangent applies. Had the list
                      // been clipped instead, do_extrapolate would be
                      // false and this goes unused.
                      prev_end_slope = layer_end_slope(*prev_layer);
                    }
                    else
                    {
                      ...
                      // And so stays flat where it is extrapolated.
                      prev_end_slope = 0;
                    }
            ...
            else
              {
                ...
                add_region_contribution_to_mesh(mesh,
                                                ...
                                                NAN,
                                                flat_list,
                                                NAN, // the round profile carries its own end point
                                                r);
    ...
    for (auto& mesh : meshes)
        merge_by_distance(mesh, this->merge_distance); 
                                    ...
                                    string& regions_giv)
{
    ...
    for (auto it = skeleton->halfedges_begin(); it != skeleton->halfedges_end(); ++it) {
        ...
        // Cleanup the polygons. Skeleton vertices can coincide - several
        // bisectors meeting at one point, or simply landing on each other
        // numerically - which leaves zero length edges. A polygon with one
        // is not simple, and skeleton_to_mesh() drops non simple regions
        // outright, punching a hole clean through the solid. Collapse them
        // here instead.
        double eps2 = 1e-10;
        for (size_t i=0; i+1 < poly.size(); )
        {
            if (CGAL::squared_distance(poly[i], poly[i+1]) < eps2)
                poly.erase(poly.begin()+i+1);
            else
                i++;
        }
        while (poly.size() > 1
               && CGAL::squared_distance(poly[poly.size()-1], poly[0]) < eps2)
            poly.erase(poly.end()-1);
